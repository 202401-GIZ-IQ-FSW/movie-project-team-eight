...
import './styles.css';
import { SearchIcon, ChevronDownIcon, MenuIcon, XIcon } from '@heroicons/react/solid';
  ...
  return (
    <header className='bg-black bg-opacity-80 py-3 px-6 sm:px-10 font-sans-serif shadow-lg fixed w-full z-50 top-0 transition-colors duration-300'>
      <div className='flex flex-wrap items-center justify-between gap-4'>
        <a href="/" className='text-red-600 text-xl font-bold'>CatFlix</a>
        ...
        <ul className={`lg:flex ${isMenuOpen ? 'flex' : 'hidden'} flex-col lg:flex-row lg:space-x-8`}>
          <li>
            <a href="#" className='font-bold text-gray-300 hover:text-red-600 text-15px flex items-center'>Home</a>
          </li>
          <li className='relative group'>
            <a href="#" className='text-gray-300 hover:text-red-600 text-15px flex items-center'>
            ...
            <div className="dropdown absolute left-0 hidden group-hover:block bg-gray-700 z-20 overflow-y-auto max-h-48">
              {genres.map((genre) => (
                <a key={genre.id} href="#" className="text-white block px-4 py-2 text-sm hover:bg-red-600">{genre.name}</a>
              ))}
            </div>
          </li>
          <li className='relative group'>
            ...
            <div className="dropdown absolute left-0 hidden group-hover:block bg-gray-700 z-20">
              {moviesOptions.map((option, index) => (
                <a key={index} href={`/movies/${option.key}`} className="text-white block px-4 py-2 text-sm hover:bg-red-600">{option.name}</a>
              ))}
            </div>
          </li>
          <li>
